...
let CLDR_DATES_DIR = path.dirname(
  require.resolve('cldr-dates-full/package.json')
);
let CLDR_NUMBERS_DIR = path.dirname(
  require.resolve('cldr-numbers-full/package.json')
);
...
let CALENDARS_LOCALES_HASH = glob
  .sync('*/ca-*.json', {
...
let CURRENCIES_LOCALES_HASH = glob
  .sync('*/currencies.json', {
    cwd: path.resolve(CLDR_NUMBERS_DIR, 'main'),
  })
  .reduce((hash, filename) => {
    hash[path.dirname(filename)] = true;
...
let DEFAULT_CONTENT_ARRAY =
  require('cldr-core/defaultContent.json').defaultContent.map((value) => {
...
let ALL_LOCALES_HASH = Object.keys(PARENT_LOCALES_HASH)
...
function hasNumbersFields(locale) {
  return (
    NUMBERS_LOCALES_HASH.hasOwnProperty(normalizeLocale(locale)) &&
